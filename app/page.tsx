...
function mc(m: number) { return m >= 50 ? T.green : m >= 35 ? T.yellow : T.red; }
...
const INV_CONFIG: Record<string, { label: string; color: string; bg: string }> = {
  none: { label: 'No Invoice', color: '#888888', bg: '#333333' },
  draft: { label: 'Draft', color: '#888888', bg: '#333333' },
...
function Chip({ children, color, bg, small }: { children: React.ReactNode; color?: string; bg?: string; small?: boolean }) {
  return (
    <span style={{ display: 'inline-flex', alignItems: 'center', gap: 3, background: bg || '#333', color: color || T.text1, padding: small ? '2px 6px' : '4px 8px', borderRadius: 4, fontSize: small ? 10 : 11, fontWeight: 500, whiteSpace: 'nowrap' }}>
      {children}
    </span>
  );
}
...
function SLAChip({ job }: { job: Job }) {
  ...
  const bg = rem < 0 ? 'rgba(244,71,71,0.12)' : rem <= 1 ? 'rgba(220,220,170,0.12)' : 'rgba(106,153,85,0.12)';
...
function MarginBar({ job }: { job: Job }) {
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
      <div style={{ flex: 1, height: 3, borderRadius: 1, background: T.border }}>
...
function CreateJobModal({ onClose, onCreate }: { onClose: () => void; onCreate: (job: Job) => void }) {
  ...
  const inputStyle: React.CSSProperties = { background: T.bg2, border: '1px solid ' + T.border, borderRadius: 8, padding: '10px 14px', fontSize: 14, color: T.text0, width: '100%', outline: 'none', boxSizing: 'border-box' };
  const labelStyle: React.CSSProperties = { fontSize: 14, color: '#888888', fontWeight: 400, marginBottom: 4, display: 'block' };
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', zIndex: 400, display: 'flex', alignItems: 'center', justifyContent: 'center' }} onClick={onClose}>
      <div style={{ background: T.bg1, border: '1px solid ' + T.border, borderRadius: 0, padding: 0, width: 480, maxHeight: '90vh', overflowY: 'auto', boxShadow: '0 5px 30px rgba(0,0,0,0.5)' }} onClick={(e) => e.stopPropagation()}>
        <div style={{ padding: '14px 20px', borderBottom: '1px solid ' + T.border }}>
          <h2 style={{ fontSize: 18, fontWeight: 600, color: '#ffffff', margin: 0 }}>New Job</h2>
        </div>
        ...
        <div style={{ background: '#1a1a1a', borderRadius: 12, padding: 12, marginBottom: 16, display: 'flex', justifyContent: 'space-between', alignItems: 'center', border: '1px solid #333' }}>
          <span style={{ fontSize: 14, color: '#888888' }}>Quoted Price</span>
          <span style={{ fontSize: 22, fontWeight: 600, color: T.accent }}>{$(quoted)}</span>
        </div>

        <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
          <button onClick={onClose} style={{ background: '#333333', color: T.text1, border: '1px solid #555', borderRadius: 6, padding: '6px 14px', fontSize: 14, cursor: 'pointer' }}>Cancel</button>
          <button onClick={handleCreate} style={{ background: '#6366f1', color: '#fff', border: 'none', borderRadius: 8, padding: '6px 20px', fontSize: 14, fontWeight: 500, cursor: 'pointer' }}>Create Job</button>
        </div>
        </div>
      </div>
    </div>
...
function JobPanel({ job, onClose, onMove }: { job: Job; onClose: () => void; onMove: (id: string, status: string) => void }) {
  ...
  return (
    <div style={{ background: T.bg1, border: 'none', borderRadius: 0, display: 'flex', flexDirection: 'column', height: '100%', overflow: 'hidden' }}>
      <div style={{ padding: '16px 20px', borderBottom: '1px solid ' + T.border }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexWrap: 'wrap' }}>
            <span style={{ fontSize: 10, color: T.text2, fontWeight: 700, fontFamily: 'monospace' }}>{job.id}</span>
            <Chip color={st.color} bg={st.bg}><Dot color={st.color} size={5} /> {st.label}</Chip>
            <InvBadge status={job.invoice} />
            {job.recurring && <Chip color={T.blue} bg="rgba(86,156,214,0.12)" small>Recurring</Chip>}
          </div>
          <button onClick={onClose} style={{ background: '#333333', border: '1px solid #555', color: T.text2, fontSize: 14, cursor: 'pointer', lineHeight: 1, width: 26, height: 26, borderRadius: 6, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>×</button>
        </div>
        <h2 style={{ fontSize: 18, fontWeight: 600, color: '#ffffff', margin: '0 0 3px' }}>{job.client}</h2>
        <div style={{ fontSize: 14, color: '#888888' }}>{job.contact} · {job.phone}</div>
        {next && (
          <button onClick={() => onMove(job.id, next.key)} style={{ marginTop: 10, background: '#6366f1', color: '#fff', border: 'none', borderRadius: 8, padding: '8px 16px', fontSize: 14, fontWeight: 600, cursor: 'pointer', width: '100%' }}>
            Move to {next.label} →
          </button>
        )}
      </div>
      ...
      <div style={{ display: 'flex', borderBottom: '1px solid ' + T.border, padding: '0 20px' }}>
        {tabs.map((t) => (
          <div key={t} onClick={() => setTab(t)} style={{ padding: '10px 14px', fontSize: 12, fontWeight: tab === t ? 600 : 400, color: tab === t ? T.accent : T.text2, borderBottom: tab === t ? '2px solid ' + T.accent : '2px solid transparent', cursor: 'pointer', textTransform: 'capitalize' }}>
            {t}
          </div>
        ))}
      </div>
      ...
      <div style={{ padding: '16px 20px', flex: 1, overflowY: 'auto' }}>
        {tab === 'overview' && (
          <div>
            <div style={{ background: '#1a1a1a', borderRadius: 12, padding: '20px', marginBottom: 16, display: 'grid', gridTemplateColumns: '1fr 1fr 1fr 1fr', gap: 6, border: '1px solid #333' }}>
              {[
                { l: 'Quoted', v: $(p.rev), c: T.accent },
                { l: 'Cost', v: $(p.cost), c: T.text1 },
                { l: 'Profit', v: (p.net < 0 ? '-' : '') + $(p.net), c: p.net >= 0 ? T.green : T.red },
                { l: 'Margin', v: p.margin.toFixed(1) + '%', c: mc(p.margin) },
              ].map((m, i) => (
                <div key={i} style={{ textAlign: 'center' }}>
                  <div style={{ fontSize: 14, color: '#888888', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: 2 }}>{m.l}</div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginBottom: 16 }}>
              {[
                ...
                <div key={i} style={{ background: '#1a1a1a', borderRadius: 8, padding: '8px 10px', border: '1px solid #333' }}>
                  <div style={{ fontSize: 14, color: '#888888', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.04em', marginBottom: 2 }}>{item.l}</div>
                  {item.chip ? <SLAChip job={job} /> : <div style={{ fontSize: 14, color: T.text0, fontWeight: 500 }}>{item.v}</div>}
                </div>
              ))}
            </div>

            <div style={{ fontSize: 11, fontWeight: 600, color: T.text1, marginBottom: 8, textTransform: 'uppercase', letterSpacing: '0.05em' }}>Team</div>
            <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 16 }}>
              {[job.shooter, job.editor, ...(job.timeLogs || []).map((l) => l.person)]
                ...
                .map((id) => {
                  ...
                  return (
                    <div key={id} style={{ display: 'flex', alignItems: 'center', gap: 6, background: '#333', borderRadius: 4, padding: '6px 10px', border: '1px solid ' + T.border2 }}>
                      <Av id={id} size={22} />
                      <div>
                        <div style={{ fontSize: 11, color: T.text0, fontWeight: 600 }}>{pr.name}</div>
                        <div style={{ fontSize: 9, color: '#888888' }}>{hrs > 0 ? hrs.toFixed(1) + 'h' : 'Assigned'}</div>
                      </div>
                    </div>
                  );
                })}
            ...
            {job.notes && (
              <div style={{ marginBottom: 16 }}>
                <div style={{ fontSize: 11, fontWeight: 600, color: T.text1, marginBottom: 6, textTransform: 'uppercase', letterSpacing: '0.05em' }}>Notes</div>
                <div style={{ fontSize: 14, color: T.text1, background: '#1a1a1a', borderRadius: 8, padding: 10, lineHeight: 1.6, border: '1px solid #333' }}>{job.notes}</div>
              </div>
            )}
          </div>
        )}
        ...
        {tab === 'call' && job.callSummary && (
          <div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 10 }}>
              ...
              <span style={{ fontSize: 11, fontWeight: 600, color: T.text0 }}>Qualification Call Summary</span>
            </div>
            <div style={{ background: '#1a1a1a', borderRadius: 8, padding: 12, fontSize: 14, color: T.text1, lineHeight: 1.6, borderLeft: '3px solid ' + SRC_COLOR.fireflies, border: '1px solid #333' }}>
              {job.callSummary}
            </div>
          </div>
        )}
          ...
          <div>
            <div style={{ marginBottom: 14 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 10, marginBottom: 3 }}>
                <span style={{ color: '#888888' }}>Budget consumed</span>
              ...
              <div style={{ height: 6, borderRadius: 3, background: T.border }}>
                <div style={{ width: Math.min(p.rev > 0 ? (p.cost / p.rev) * 100 : 0, 100) + '%', height: '100%', borderRadius: 3, background: p.rev > 0 && p.cost / p.rev > 0.7 ? T.red : p.cost / p.rev > 0.5 ? T.yellow : T.green }} />
              </div>
            </div>

            {(job.timeLogs || []).length > 0 && (
              <div style={{ marginBottom: 14 }}>
                <div style={{ fontSize: 10, fontWeight: 600, color: T.text1, marginBottom: 6 }}>Time Logged (EverSense)</div>
                {job.timeLogs.map((l, i) => {
                  ...
                  return (
                    <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '4px 0', borderBottom: '1px solid ' + T.border }}>
                      ...
                      <span style={{ fontSize: 10, color: T.text1, flex: 1 }}>{pr ? pr.name : ''} — {l.task}</span>
                      <span style={{ fontSize: 10, color: '#888888' }}>{l.hours}h</span>
                ...
                <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 12, padding: '6px 0', fontSize: 10, fontWeight: 700 }}>
                  <span style={{ color: '#888888' }}>Labour: {$x(c.labour)}</span>
                  {c.expenses > 0 && <span style={{ color: '#888888' }}>Exp: {$x(c.expenses)}</span>}
            ...
            {(job.expenses || []).length > 0 && (
              <div style={{ marginBottom: 14 }}>
                <div style={{ fontSize: 10, fontWeight: 600, color: T.text1, marginBottom: 6 }}>Expenses</div>
                {job.expenses.map((e, i) => (
                  <div key={i} style={{ display: 'flex', justifyContent: 'space-between', padding: '4px 0', borderBottom: '1px solid ' + T.border, fontSize: 11 }}>
                    <span style={{ color: T.text1 }}>{e.desc}</span>
            ...
            <div style={{ marginBottom: 14 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 6 }}>
                <SrcIcon src="xero" />
                <span style={{ fontSize: 10, fontWeight: 600, color: T.text1 }}>Invoice (Xero)</span>
              </div>
              <div style={{ background: '#1a1a1a', borderRadius: 8, padding: 10, display: 'flex', justifyContent: 'space-between', alignItems: 'center', border: '1px solid #333' }}>
                <InvBadge status={job.invoice} />
                <span style={{ fontSize: 14, fontWeight: 600, color: T.accent }}>{$(job.quoted)}</span>
              </div>
            </div>
          </div>
        )}
            ...
            {(job.activity || []).slice().reverse().map((a, i) => (
              <div key={i} style={{ display: 'flex', gap: 8, marginBottom: 0 }}>
                ...
                <div style={{ paddingBottom: 12 }}>
                  <div style={{ fontSize: 14, color: T.text0 }}>{a.text}</div>
                  <div style={{ fontSize: 9, color: T.text2, marginTop: 1 }}>{a.date} via {a.src}</div>
                </div>
              </div>
            ))}
          </div>
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: 24 }}>
        <div>
          <h1 style={{ fontSize: 26, fontWeight: 600, color: '#ffffff', margin: 0 }}>Today</h1>
          <p style={{ color: '#888888', fontSize: 14, margin: '4px 0 0' }}>Thursday 27 March 2026</p>
        </div>
      </div>

      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr 1fr', gap: 10, marginBottom: 20 }}>
        {[
          { l: 'Revenue', v: $(rev), c: T.text0 },
          ...
          { l: 'Outstanding', v: $(outstanding), c: T.yellow },
        ].map((m, i) => (
          <div key={i} style={{ background: '#1a1a1a', border: '1px solid #333', borderRadius: 12, padding: '20px' }}>
            <div style={{ fontSize: 14, color: '#888888', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.06em' }}>{m.l}</div>
            <div style={{ fontSize: 22, fontWeight: 600, color: m.c, marginTop: 4 }}>{m.v}</div>
            {m.s && <div style={{ fontSize: 10, color: '#888888', marginTop: 2 }}>{m.s}</div>}
          </div>
        ))}
      </div>

      {overdue.length > 0 && (
        <div style={{ background: 'rgba(244,71,71,0.08)', border: '1px solid rgba(244,71,71,0.2)', borderRadius: 8, padding: '10px 16px', marginBottom: 16, display: 'flex', alignItems: 'center', gap: 8 }}>
          <Dot color={T.red} size={8} />
          <span style={{ color: T.red, fontSize: 14, fontWeight: 600 }}>{overdue.length} overdue</span>
          <span style={{ color: '#888888', fontSize: 14 }}>— {overdue.map((j) => j.client).join(', ')}</span>
        </div>
      )}

      {inProgress.length > 0 && (
        <div style={{ marginBottom: 20 }}>
          <div style={{ fontSize: 18, fontWeight: 600, color: '#ffffff', marginBottom: 10, display: 'flex', alignItems: 'center', gap: 6 }}>
          ...
          {inProgress.map((j) => {
            ...
            void pr;
            return (
              <div key={j.id} onClick={() => onSelect(j)} style={{ background: '#1a1a1a', border: '1px solid #333', borderRadius: 8, padding: 12, marginBottom: 8, cursor: 'pointer', borderLeft: '3px solid ' + getSt(j.status).color, boxShadow: '0 2px 4px rgba(0,0,0,0.3)' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6, flexWrap: 'wrap' }}>
                  <span style={{ fontSize: 10, color: T.text2, fontWeight: 700, fontFamily: 'monospace' }}>{j.id}</span>
                  ...
                  <span style={{ marginLeft: 'auto', fontSize: 14, fontWeight: 600, color: T.accent }}>{$(j.quoted)}</span>
                </div>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                  <div>
                    <div style={{ fontSize: 14, fontWeight: 600, color: T.text0 }}>{j.client}</div>
                    <div style={{ fontSize: 14, color: '#888888', marginTop: 1 }}>{j.location}</div>
                  </div>
      ...
      {needsAttention.length > 0 && (
        <div style={{ marginBottom: 20 }}>
          <div style={{ fontSize: 18, fontWeight: 600, color: '#ffffff', marginBottom: 10, display: 'flex', alignItems: 'center', gap: 6 }}>
            <Dot color={T.yellow} size={7} /> Needs Attention
          </div>
          {needsAttention.map((j) => (
            <div key={j.id} onClick={() => onSelect(j)} style={{ background: '#1a1a1a', border: '1px solid #333', borderRadius: 8, padding: '10px 14px', marginBottom: 6, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 10 }}>
              <Chip color={getSt(j.status).color} bg={getSt(j.status).bg} small><Dot color={getSt(j.status).color} size={4} /> {getSt(j.status).label}</Chip>
              <span style={{ fontSize: 14, color: T.text0, flex: 1, fontWeight: 500 }}>{j.client}</span>
              <SLAChip job={j} />
              <InvBadge status={j.invoice} />
              <span style={{ fontSize: 14, fontWeight: 600, color: T.accent }}>{$(j.quoted)}</span>
            </div>
          ))}
        </div>
      )}
      ...
      <div>
        <div style={{ fontSize: 18, fontWeight: 600, color: '#ffffff', marginBottom: 10 }}>Upcoming</div>
        {jobs.filter((j) => ['booked', 'scheduled'].includes(j.status)).sort((a, b) => new Date(a.scheduledDate).getTime() - new Date(b.scheduledDate).getTime()).map((j) => (
          <div key={j.id} onClick={() => onSelect(j)} style={{ background: '#1a1a1a', border: '1px solid #333', borderRadius: 8, padding: '10px 14px', marginBottom: 6, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 10 }}>
            ...
            <span style={{ fontSize: 14, color: T.text0, flex: 1, fontWeight: 500 }}>{j.client}</span>
            <span style={{ fontSize: 10, color: '#888888' }}>{j.scheduledDate}</span>
...
function BoardPage({ jobs, onSelect }: { jobs: Job[]; onSelect: (job: Job) => void }) {
  ...
  return (
    <div>
      <h1 style={{ fontSize: 24, fontWeight: 600, color: '#ffffff', margin: '0 0 18px' }}>Job Board</h1>
        ...
        {cols.map((col) => {
          ...
          return (
            <div key={col} style={{ minWidth: 210, flex: '0 0 220px' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 8, padding: '6px 10px', background: st.bg, borderRadius: 4, border: '1px solid #333' }}>
                <Dot color={st.color} size={7} />
                <span style={{ fontSize: 11, fontWeight: 600, color: T.text0 }}>{st.label}</span>
                <span style={{ fontSize: 10, color: '#888888', marginLeft: 'auto', background: '#333', padding: '1px 7px', borderRadius: 10, fontWeight: 700 }}>{cj.length}</span>
              </div>
              {cj.map((j) => (
                <div key={j.id} onClick={() => onSelect(j)} style={{ background: '#1a1a1a', border: '1px solid #333', borderRadius: 8, padding: 12, marginBottom: 6, cursor: 'pointer', borderLeft: '3px solid ' + st.color, boxShadow: '0 2px 4px rgba(0,0,0,0.3)' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
                    <span style={{ fontSize: 9, color: T.text2, fontWeight: 700, fontFamily: 'monospace' }}>{j.id}</span>
                    <SLAChip job={j} />
                  </div>
                  <div style={{ fontSize: 14, fontWeight: 600, color: T.text0, marginBottom: 6 }}>{j.client}</div>
                  <MarginBar job={j} />
...
function JobsPage({ jobs, onSelect }: { jobs: Job[]; onSelect: (job: Job) => void }) {
  ...
  return (
    <div>
      <h1 style={{ fontSize: 24, fontWeight: 600, color: '#ffffff', margin: '0 0 16px' }}>All Jobs ({filtered.length})</h1>
      <div style={{ display: 'flex', gap: 8, marginBottom: 14 }}>
        <input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Search client or job ID..." style={{ background: T.bg2, border: '1px solid ' + T.border, borderRadius: 8, padding: '10px 14px', fontSize: 14, color: T.text0, width: 240, outline: 'none' }} />
        <select value={fs} onChange={(e) => setFs(e.target.value)} style={{ background: T.bg2, border: '1px solid ' + T.border, borderRadius: 8, padding: '10px 14px', fontSize: 14, color: T.text1, outline: 'none' }}>
      ...
      <div style={{ background: '#1a1a1a', border: '1px solid #333', borderRadius: 12, overflow: 'hidden' }}>
        <div style={{ display: 'grid', gridTemplateColumns: '56px 90px 1fr 80px 72px 60px 36px', gap: 0, padding: '10px 16px', borderBottom: '1px solid ' + T.border }}>
          {['ID', 'Status', 'Client', 'Invoice', 'Quoted', 'Margin', ''].map((h, i) => (
            <span key={i} style={{ fontSize: 14, color: '#888888', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.06em' }}>{h}</span>
          ))}
        </div>
        {filtered.map((j) => {
          const pr = jProfit(j);
          ...
          return (
            <div key={j.id} onClick={() => onSelect(j)} style={{ display: 'grid', gridTemplateColumns: '56px 90px 1fr 80px 72px 60px 36px', alignItems: 'center', gap: 0, padding: '10px 16px', borderBottom: '1px solid ' + T.border, cursor: 'pointer' }}>
              <span style={{ fontSize: 10, color: T.text2, fontWeight: 700, fontFamily: 'monospace' }}>{j.id}</span>
              <Chip color={st.color} bg={st.bg} small><Dot color={st.color} size={4} /> {st.label}</Chip>
              <div>
                <div style={{ fontSize: 14, color: T.text0, fontWeight: 500 }}>{j.client}</div>
                <div style={{ fontSize: 10, color: '#888888' }}>{j.scheduledDate} · {getTy(j.type).label}</div>
              </div>
              <InvBadge status={j.invoice} />
              <span style={{ fontSize: 14, fontWeight: 600, color: T.accent }}>{$(j.quoted)}</span>
  ...
  return (
    <div>
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 18 }}>
        <SrcIcon src="xero" />
        <h1 style={{ fontSize: 24, fontWeight: 600, color: '#ffffff', margin: 0 }}>Finance</h1>
        <span style={{ fontSize: 14, color: '#888888' }}>March 2026</span>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr 1fr 1fr', gap: 10, marginBottom: 18 }}>
        {[
          ...
          { l: 'Awaiting', v: $(outs), c: T.yellow },
        ].map((m, i) => (
          <div key={i} style={{ background: '#1a1a1a', border: '1px solid #333', borderRadius: 12, padding: '20px' }}>
            <div style={{ fontSize: 14, color: '#888888', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em' }}>{m.l}</div>
            <div style={{ fontSize: 22, fontWeight: 600, color: m.c, marginTop: 4 }}>{m.v}</div>
          </div>
        ))}
      </div>

      <div style={{ background: '#1a1a1a', border: '1px solid #333', borderRadius: 12, padding: 18, marginBottom: 16 }}>
        <div style={{ fontSize: 18, fontWeight: 600, color: '#ffffff', marginBottom: 12 }}>Revenue by Service</div>
        {Object.entries(bt).sort((a, b) => b[1].rev - a[1].rev).map(([type, d]) => {
          const m = d.rev > 0 ? (d.profit / d.rev) * 100 : 0;
          return (
            <div key={type} style={{ marginBottom: 8 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 10, marginBottom: 2 }}>
                <span style={{ color: T.text0 }}>{getTy(type).label} ×{d.cnt}</span>
              ...
              <div style={{ height: 4, borderRadius: 2, background: T.border }}>
                <div style={{ width: (d.rev / mx) * 100 + '%', height: '100%', borderRadius: 2, background: T.accent }} />
              </div>
            </div>
          );
        })}
      ...
      <div style={{ background: '#1a1a1a', border: '1px solid #333', borderRadius: 12, overflow: 'hidden' }}>
        <div style={{ fontSize: 18, fontWeight: 600, color: '#ffffff', padding: '12px 16px', borderBottom: '1px solid ' + T.border }}>Job P&L</div>
        {jobs.sort((a, b) => new Date(b.created).getTime() - new Date(a.created).getTime()).map((j) => {
          ...
          return (
            <div key={j.id} onClick={() => onSelect(j)} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '7px 14px', borderBottom: '1px solid ' + T.border, cursor: 'pointer' }}>
              <span style={{ fontSize: 9, color: T.text2, fontWeight: 600, minWidth: 50 }}>{j.id}</span>
              <span style={{ fontSize: 14, color: T.text0, flex: 1 }}>{j.client}</span>
              <InvBadge status={j.invoice} />
...
function QuoteSensePage() {
  ...
  const inputStyle: React.CSSProperties = { background: T.bg2, border: '1px solid ' + T.border, borderRadius: 8, padding: '10px 14px', fontSize: 14, color: T.text0, outline: 'none', width: '100%', boxSizing: 'border-box' };

  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: 16 }}>
        <div>
          <h1 style={{ fontSize: 24, fontWeight: 600, color: '#ffffff', margin: 0 }}>QuoteSense</h1>
          <p style={{ color: '#888888', fontSize: 14, margin: '4px 0 0' }}>Build quotes with live margin tracking</p>
        </div>
        <label style={{ display: 'flex', alignItems: 'center', gap: 4, fontSize: 10, color: '#888888', cursor: 'pointer' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
        ...
        <div>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginBottom: 12 }}>
            <div>
              <label style={{ fontSize: 14, color: '#888888', fontWeight: 600, textTransform: 'uppercase' as const, display: 'block', marginBottom: 3 }}>Client</label>
              <input value={clientName} onChange={(e) => setClientName(e.target.value)} placeholder="Client name" style={inputStyle} />
            </div>
            <div>
              <label style={{ fontSize: 14, color: '#888888', fontWeight: 600, textTransform: 'uppercase' as const, display: 'block', marginBottom: 3 }}>Email</label>
          ...
          {SERVICES.map((cat) => (
            <div key={cat.cat} style={{ marginBottom: 8 }}>
              <div onClick={() => setExpandedCat(expandedCat === cat.cat ? '' : cat.cat)} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '8px 10px', background: '#1a1a1a', border: '1px solid #333', borderRadius: 8, cursor: 'pointer', marginBottom: 2 }}>
                <span style={{ fontSize: 10, fontWeight: 700, color: T.accent, letterSpacing: '0.06em' }}>{cat.cat}</span>
                <span style={{ fontSize: 10, color: T.text2 }}>{expandedCat === cat.cat ? '▾' : '▸'} {cat.items.length}</span>
              </div>
              {expandedCat === cat.cat && (
                <div style={{ border: '1px solid #333', borderRadius: 8, overflow: 'hidden' }}>
                  {cat.items.map((svc) => {
                    ...
                    return (
                      <div key={svc.id} onClick={() => !inQuote && addItem(svc)} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '8px 10px', borderBottom: '1px solid ' + T.border, cursor: inQuote ? 'default' : 'pointer', background: inQuote ? 'rgba(99,102,241,0.06)' : 'transparent', opacity: inQuote ? 0.7 : 1 }}>
                        <div style={{ flex: 1 }}>
                          <div style={{ fontSize: 14, color: T.text0, fontWeight: 500 }}>{svc.label}</div>
                          <div style={{ fontSize: 9, color: T.text2, marginTop: 1 }}>{svc.team} · {svc.hours}</div>
                          {svc.note && <div style={{ fontSize: 8, color: T.yellow, marginTop: 1 }}>{svc.note}</div>}
                        </div>
                        <div style={{ textAlign: 'right', minWidth: 70 }}>
                          {showStandard && <div style={{ fontSize: 9, color: T.text2, textDecoration: 'line-through' }}>{$(svc.stdRate)}</div>}
                          <div style={{ fontSize: 14, fontWeight: 600, color: T.accent }}>{$(svc.price)}</div>
                          <div style={{ fontSize: 8, color: mc(svcMargin), fontWeight: 600 }}>{svcMargin.toFixed(0)}% margin</div>
                        </div>
                        <div style={{ width: 24, height: 24, borderRadius: 4, background: inQuote ? T.green : T.primary, display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', fontSize: inQuote ? 10 : 14, fontWeight: 700, flexShrink: 0 }}>
                          {inQuote ? '✓' : '+'}
                        </div>
                      </div>
                    );
        ...
        {/* RIGHT — Quote Builder */}
        <div>
          <div style={{ background: '#1a1a1a', border: '1px solid #333', borderRadius: 12, padding: 16, position: 'sticky', top: 20 }}>
            <div style={{ fontSize: 14, fontWeight: 600, color: T.text0, marginBottom: 4 }}>{clientName || 'New Quote'}</div>
            {clientEmail && <div style={{ fontSize: 10, color: '#888888', marginBottom: 12 }}>{clientEmail}</div>}

            {items.length === 0 && (
              <div style={{ padding: '30px 0', textAlign: 'center', color: T.text2, fontSize: 14 }}>
            ...
            {items.length > 0 && (
              <div>
                {items.map((item) => {
                  ...
                  return (
                    <div key={item.id} style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '8px 0', borderBottom: '1px solid ' + T.border }}>
                      <div style={{ flex: 1 }}>
                        <div style={{ fontSize: 14, color: T.text0 }}>{item.label}</div>
                        {showStandard && <div style={{ fontSize: 9, color: T.text2, textDecoration: 'line-through' }}>{$(lineStd)}</div>}
                      </div>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                        <button onClick={() => updateQty(item.id, item.qty - 1)} style={{ width: 20, height: 20, borderRadius: 4, background: '#333333', border: '1px solid #555', color: T.text1, fontSize: 12, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>-</button>
                        <span style={{ fontSize: 11, color: T.text0, minWidth: 16, textAlign: 'center' }}>{item.qty}</span>
                        <button onClick={() => updateQty(item.id, item.qty + 1)} style={{ width: 20, height: 20, borderRadius: 4, background: '#333333', border: '1px solid #555', color: T.text1, fontSize: 12, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>+</button>
                      </div>
                      <span style={{ fontSize: 14, fontWeight: 600, color: T.accent, minWidth: 56, textAlign: 'right' }}>{$(lineTotal)}</span>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '10px 0', borderBottom: '1px solid ' + T.border }}>
                  <span style={{ fontSize: 10, color: '#888888', flex: 1 }}>Discount %</span>
                ...
                <div style={{ paddingTop: 12 }}>
                  {showStandard && (
                    <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
                      <span style={{ fontSize: 10, color: T.text2 }}>Standard Rate</span>
                      <span style={{ fontSize: 14, color: T.text2, textDecoration: 'line-through' }}>{$(totalStd)}</span>
                    </div>
                  )}
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 8 }}>
                    <span style={{ fontSize: 14, fontWeight: 600, color: T.text0 }}>TOTAL</span>
                    <span style={{ fontSize: 22, fontWeight: 600, color: T.accent }}>{$(total)}</span>
                  </div>
                  {showStandard && (
                    <div style={{ background: 'rgba(106,153,85,0.12)', borderRadius: 4, padding: '6px 10px', textAlign: 'center', marginBottom: 10 }}>
                ...
                {/* Internal P&L */}
                <div style={{ background: T.bg0, borderRadius: 8, padding: 10, marginBottom: 12, border: '1px solid #333' }}>
                  <div style={{ fontSize: 9, color: T.text2, fontWeight: 600, textTransform: 'uppercase', marginBottom: 6 }}>INTERNAL — not shown to client</div>
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr 1fr', gap: 4 }}>
                    {[
                      { l: 'Revenue', v: $(total), c: T.text0 },
                      { l: 'Cost', v: $(totalCost), c: T.text1 },
                      { l: 'Profit', v: (afterTaxProfit < 0 ? '-' : '') + $(afterTaxProfit), c: afterTaxProfit >= 0 ? T.green : T.red },
                      { l: 'Margin', v: margin.toFixed(1) + '%', c: mc(margin) },
                    ].map((m, i) => (
                      <div key={i}>
                        <div style={{ fontSize: 7, color: T.text2, fontWeight: 600, textTransform: 'uppercase' }}>{m.l}</div>
                  ...
                  {margin < 50 && margin > 0 && (
                    <div style={{ fontSize: 9, color: T.yellow, marginTop: 4 }}>Below 50% target. Raise total to hit margin goal.</div>
                  )}
                  {margin < 0 && (
                ...
                <div style={{ marginBottom: 12 }}>
                  <div style={{ fontSize: 9, color: T.text2, fontWeight: 600, textTransform: 'uppercase', marginBottom: 4 }}>Team required</div>
                  {items.map((item) => (
                    <div key={item.id} style={{ fontSize: 10, color: '#888888', padding: '2px 0' }}>
                ...
                <div style={{ display: 'flex', gap: 6 }}>
                  <button style={{ flex: 1, background: '#6366f1', color: '#fff', border: 'none', borderRadius: 8, padding: '10px 0', fontSize: 14, fontWeight: 600, cursor: 'pointer' }}>
                    Create Job from Quote
                  </button>
                  <button style={{ background: '#333333', color: T.text1, border: '1px solid #555', borderRadius: 6, padding: '10px 14px', fontSize: 11, cursor: 'pointer' }}>
                    PDF
                  </button>
                  <button onClick={() => { setItems([]); setDiscount(0); setClientName(''); setClientEmail(''); }} style={{ background: '#333333', color: T.text2, border: '1px solid #555', borderRadius: 6, padding: '10px 14px', fontSize: 11, cursor: 'pointer' }}>
                    Clear
                  </button>
                </div>
              </div>
...
export default function JobSensePage() {
  ...
  return (
    <div style={{ height: '100vh', display: 'flex', flexDirection: 'column', background: T.bg0, fontFamily: "-apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif", color: T.text1 }}>
      <div style={{ flex: 1, display: 'flex', overflow: 'hidden' }}>
      {/* Sidebar */}
      <div style={{ width: 240, background: T.bg1, borderRight: '1px solid ' + T.border, display: 'flex', flexDirection: 'column', flexShrink: 0 }}>
        {/* Brand */}
        <div style={{ padding: '20px 20px 16px', borderBottom: '1px solid ' + T.border }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            <div style={{ width: 32, height: 32, borderRadius: 8, background: '#6366f1', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 800, color: '#fff', fontSize: 15 }}>J</div>
            <div>
              <div style={{ fontWeight: 700, fontSize: 16, color: T.text0 }}>JobSense</div>
              <div style={{ fontSize: 10, color: '#888888' }}>Veblen Group</div>
            </div>
          </div>
        </div>
          ...
          {tabs.map((t) => (
            <button
              ...
              style={{
                ...
                background: page === t.id ? 'rgba(0,122,204,0.15)' : 'transparent',
                color: page === t.id ? T.accent : T.text2,
        ...
        <div style={{ padding: '12px 20px', borderTop: '1px solid ' + T.border }}>
          <button onClick={() => setShowCreate(true)} style={{ width: '100%', background: '#6366f1', color: '#fff', border: 'none', borderRadius: 8, padding: '10px 0', fontSize: 13, fontWeight: 600, cursor: 'pointer' }}>
            + New Job
          </button>
        </div>
      </div>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        {/* Top Bar */}
        <div style={{ height: 48, background: T.bg1, borderBottom: '1px solid ' + T.border, display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 24px', flexShrink: 0 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            <span style={{ fontSize: 14, fontWeight: 600, color: T.text0 }}>{activeTab.label}</span>
            <span style={{ fontSize: 11, color: '#888888', padding: '2px 8px', background: '#333', borderRadius: 4 }}>
              {jobs.filter(j => !['paid'].includes(j.status)).length} active
            </span>
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            {['HubSpot', 'Xero', 'Fireflies', 'EverSense'].map((s) => (
              <div key={s} style={{ display: 'flex', alignItems: 'center', gap: 4, fontSize: 10, color: '#888888' }}>
        ...
        <div style={{ flex: 1, display: 'flex', overflow: 'hidden' }}>
          ...
          {/* Right Detail Panel */}
          {sel && (
            <div style={{ width: 400, background: T.bg1, borderLeft: '1px solid ' + T.border, display: 'flex', flexDirection: 'column', flexShrink: 0, overflow: 'hidden' }}>
      ...
      {/* Status Bar */}
      <div style={{ height: 22, background: T.accent, display: 'flex', alignItems: 'center', padding: '0 10px', fontSize: 11, color: '#fff', flexShrink: 0 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
